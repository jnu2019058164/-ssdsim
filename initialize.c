...
struct ssd_info *initiation(struct ssd_info *ssd)
{
	...
	printf("\ninput trace file name:");
	//const char* str_tmp = "test.ascii";
	//strcpy(ssd->tracefilename,str_tmp);
...
struct parameter_value *load_parameters(char parameter_file[30])
{
	...
	while(fgets(buf,200,fp)){
		...
		//TAG
		//read new parameter
		else if((res_eql=strcmp(buf,"flash_type")) ==0){
			sscanf(buf + next_eql,"%d",&p->flash_type);
		}
		else if((res_eql=strncmp(buf,"t_PXLC",6)) ==0){
			sscanf(buf+7,"%d",&i);
			sscanf(buf + next_eql,"%d",&p->time_characteristics.tPXLC[i]); 
		}
		else if((res_eql=strncmp(buf,"t_RXLC",6)) ==0){
			sscanf(buf+7,"%d",&i);
			sscanf(buf + next_eql,"%d",&p->time_characteristics.tRXLC[i]); 
		}
		else if((res_eql=strncmp(buf,"slc_cache_start",15)) ==0){
			sscanf(buf+16,"%d",&i);
			sscanf(buf + next_eql,"%ld",&p->SLC_CACHE_start[i]); 
		}
		else if((res_eql=strncmp(buf,"slc_cache_end",13)) ==0){
			sscanf(buf+14,"%d",&i);
			sscanf(buf + next_eql,"%ld",&p->SLC_CACHE_end[i]); 
		}
		else if((res_eql=strcmp(buf,"slc_cache_size")) ==0){
			sscanf(buf + next_eql,"%d",&p->SLC_CACHE_SIZE);
		}
		//	
		else{
			printf("\ndon't match\t %s",buf);
	...
	//TAG
	//奇淫技巧实现XLC扩容
	//fxxking idea 4 XLC implementation
	p->page_capacity *= p->flash_type;
	p->subpage_capacity *= p->flash_type;
...
int Is_SLC_cache_blk(struct ssd_info * ssd,unsigned int chip,unsigned int die,unsigned int plane,unsigned int block){
	int slc_cache_size = (ssd->parameter->SLC_CACHE_SIZE > MAX_SLC_NUM)?MAX_SLC_NUM:ssd->parameter->SLC_CACHE_SIZE;
	unsigned int end_block;
	int64_t cache_block_num;
	int64_t *start = ssd->parameter->SLC_CACHE_start;
	int64_t *end =  ssd->parameter->SLC_CACHE_end;

	// unsigned l,h,c;
	const int64_t chip_mask = 0xff00000000000000;
	const int64_t die_mask = 0x00ff000000000000;
	const int64_t plane_mask = 0x0000ff0000000000;
	const int64_t block_mask = 0x000000007fffffff;

	int64_t chip_part = (int64_t)chip;
	int64_t die_part = (int64_t)die;
	int64_t plane_part = (int64_t)plane;
	int64_t block_part = (int64_t)block;
	int64_t c;

	chip_part<<=56;
	die_part<<=48;
	plane_part<<=40;
	c = chip_part | die_part | plane_part | block_part;
	

	// unsigned int start_chip,start_die,start_plane,start_block,end_chip,end_die,end_plane,end_block;
	int i=0,count=0;
	for(i=0;i<slc_cache_size;i++){
		// start_chip = (unsigned int)((start[i]&chip_mask)>>56);
		// start_die = (unsigned int)((start[i]&die_mask)>>48);
		// start_plane = (unsigned int)((start[i]&plane_mask)>>40);
		// start_block = (unsigned int)((start[i]&block_mask));

		// end_chip = (unsigned int)((end[i]&chip_mask)>>56);
		// end_die = (unsigned int)((end[i]&die_mask)>>48);
		// end_plane = (unsigned int)((end[i]&plane_mask)>>40);
		end_block = (unsigned int)((end[i]&block_mask));
		cache_block_num = end[i] - start[i];
		
		//禁止block 号越界
		if(end_block > ssd->parameter->block_plane){
		...
		// l = start_chip*start_die*start_plane*start_block;
		// h = end_chip*end_die*end_plane*end_block;
		// c = chip * die * plane * block;

		count += (start[i]<=c && c <=end[i])?1:0;
	}

// //code	4 debugging
// 	FILE* reqfile;
// 	reqfile=fopen("req.dat","a+");
// 	fprintf(reqfile,"%ld	",c);
// 	fclose(reqfile);
// //
